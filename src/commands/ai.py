...
class AiCommands(app_commands.Group, CommandGroup):
    ...
    async def chatgpt(self, interaction: discord.Interaction, message: str):
        ...
        try:
            answer = await gpt.interaction_chat(
                interaction, message, include_history=False
            )
        except LLMError:
            await interaction.followup.send(
                "Bir şey ters gitti, lütfen tekrar deneyin", ephemeral=True
            )
            raise
    ...
    @app_commands.command(
        name="sohbet",
        description="Botun sohbete katılmasını sağlar! (eski mesaj okur!)",
    )
    async def question(self, interaction: discord.Interaction, message: str):
        ...
        try:
            ...
            await interaction.followup.send(
                "Bir hata oluştu, lütfen tekrar deneyin", ephemeral=True
            )
...
@app_commands.allowed_contexts(guilds=True, dms=True, private_channels=True)
@app_commands.command(
    name="feridun_abi", description="Yine seks hikayesi mi yazıyorsun feridun abi?"
)
